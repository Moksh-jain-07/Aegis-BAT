...
This module covers the system design of AEGIS-BAT: how the pass-based pipeline works, how components interact, and the reasoning behind each design decision.
...
### Analysis Engine (`aegis-bat-engine` crate)
...
The `Sealed` supertrait prevents external code from implementing `AnalysisPass`. This is the sealed trait pattern — only the `aegis-bat-engine` crate can define passes:
...
**What:** The `AnalysisPass` trait requires `Sealed`, which is defined in a `mod private` block. Only types within the `aegis-bat-engine` crate can implement `Sealed`, which means only this crate can define analysis passes.
...
- Dynamic analysis escape (AEGIS-BAT is static only — it never executes the binary)
...
2. **YARA scanning** — compiling rules is expensive, but AEGIS-BAT compiles once at engine startup and reuses the compiled scanner.
...
- **Section-level entropy** — instead of byte-by-byte sliding window entropy (which is O(n * window_size)), AEGIS-BAT calculates per-section entropy, which is O(total_bytes) with a constant factor of 256 (one counter per possible byte value).
