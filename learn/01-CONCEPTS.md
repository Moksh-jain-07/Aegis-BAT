...
This module covers the security fundamentals behind static binary analysis. Every concept here maps directly to a component in AEGIS-BAT's analysis pipeline.
...
### How AEGIS-BAT Parses Them

AEGIS-BAT uses the `goblin` crate to parse all three formats through a single entry point. The `FormatPass` dispatches to format-specific parsers that extract a unified `FormatResult`:
...
**Trusting the section name.** Section names are advisory, not enforced. Malware routinely names packed code sections `.text` or `.code` to look normal. AEGIS-BAT checks entropy and permissions rather than relying on names alone.
...
AEGIS-BAT classifies entropy into five levels:
...
Entropy alone isn't enough. AEGIS-BAT combines multiple signals to detect packing:
...
**PUSHAD at entry point.** UPX and many other packers begin with `PUSHAD` (opcode `0x60`) to save all registers before the unpacking routine. AEGIS-BAT checks the first byte at the entry point offset.
...
AEGIS-BAT maintains a database of suspicious APIs, each tagged with a technique category and MITRE ATT&CK ID:
...
Import analysis isn't Windows-only. AEGIS-BAT flags Linux-specific indicators:
...
**Looking at imports in isolation.** `VirtualAlloc` is one of the most commonly used Windows APIs — every large application calls it. Only the combination with `WriteProcessMemory` and `CreateRemoteThread` is suspicious. AEGIS-BAT's combination detection avoids this false-positive trap.

**Forgetting about dynamic resolution.** Malware frequently avoids static imports by using `GetProcAddress` or `dlsym` to resolve APIs at runtime. The import table looks clean, but string analysis reveals the API names embedded in the binary. This is why AEGIS-BAT's string pass works alongside import analysis.
...
AEGIS-BAT extracts strings in multiple encodings (ASCII, UTF-16LE) and classifies them into 14 categories:
...
### AEGIS-BAT's Built-In Rules

AEGIS-BAT ships with 14 YARA rules covering major malware categories:
...
AEGIS-BAT uses `yara-x` (the Rust rewrite of YARA) rather than the original C implementation. `yara-x` is fully compatible with existing YARA rules but adds compile-time validation, better error messages, and native Rust integration without FFI overhead. The scanner compiles rules once at engine initialization and reuses the compiled ruleset for every binary.
...
AEGIS-BAT uses the `iced-x86` crate to disassemble x86 and x86_64 binaries. Starting from the entry point and any symbol table hints, it uses recursive descent disassembly: follow each instruction, and when you hit a branch, add both targets to the worklist.
...
AEGIS-BAT caps disassembly at 1,000 functions and 50,000 instructions per binary. This prevents analysis from hanging on large binaries (a full Chromium build has millions of instructions). The CFG visualization caps at 500 instructions per function to keep the frontend responsive.
...
Threat scoring takes all the individual findings from every analysis pass and produces a single 0-100 score that quantifies the binary's maliciousness. AEGIS-BAT scores across eight independent categories, each with a maximum point cap:
...
AEGIS-BAT maps findings to these ATT&CK tactics and techniques:
...
1. A binary has an overall entropy of 7.4 and only 3 imports (`GetProcAddress`, `LoadLibraryA`, `VirtualAlloc`). The entry point is in the last section, which is named `UPX1`. What can you conclude about this binary, and which AEGIS-BAT scoring categories would fire?

2. You analyze two binaries with identical SHA-256 section hashes for `.text` but different import tables. One imports `ReadFile` + `WriteFile`, the other imports `ReadFile` + `WriteFile` + `CreateRemoteThread` + `VirtualAllocEx`. How would AEGIS-BAT's threat scores differ, and why might the second binary have been modified?
